...
var galleryArea = document.querySelector('#gallery_area');
...
function createCropper(file) {
	...
	Slim.create(cropper, {
		...
		didInit: function() {
			...
			this.load(file);
			this._input.setAttribute('data-max-file-size', '1');

		},
		didRemove: function() {

			// detach from DOM
			cropper.parentNode.parentNode.parentNode.removeChild(column);
