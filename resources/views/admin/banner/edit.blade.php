...
<div class="col-12">
    <div class="box box-primary">
        ...
        <form action="{{ route('admin.banner.update', $banner->id) }}" enctype="multipart/form-data" method="post" id="addProductForm">
            @csrf
            <div class="box-body">
                ...
                <div class="form-group">
                    ...
                    <select name="page" id="page" class="form-control">
                        ...
                        <option {{ $banner->page == 'blog' ? 'selected' : '' }} value="blog" >Blog</option>
                        @foreach ($pages as $item)
                            <option {{ $item->title == $banner->page ? 'selected' : '' }} value="{{ $item->title }}">{{ $item->title }}</option>
                        @endforeach
                    </select>
                    @error('page')
                ...
                @if (!empty($banner->image))
                    <div class="form-group">
                        <img style="width: 30%" src="{{ asset('storage'.'/'.$banner->image) }}">
                        <div>
                            <a href="{{ route('admin.banner.deleteImage', ['id' => $banner->id]) }}" class="btn btn-danger">Delete</a>
                        </div>
                    </div>
                @endif

                @if ($banner->slideshow == 'yes')
                    <div class="form-group" id="slideshow" style="display: flex; justify-content: space-between; align-items: center;">
                        <label for="slideshow">Show in slideshow</label>
                        <div>
                            <label class="switch">
                                <input name="slideshow" checked type="checkbox">
                ...
                <div class="form-group">
                    ...
                    <select name="status" class="form-control">
                        ...
                        <option {{ $banner->status == 'inactive' ? 'selected' : '' }} value="inactive">Inactive</option>
                    </select>
                    @error('status')
                        <small class="text-danger" style="display: block;">
                            {{ $message }}
